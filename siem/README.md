...
## Dashboards
Two Kibana dashboards — **"Honeypot: IDS Alerts (Suricata)"** and **"Honeypot: Web Traffic & Threat Overview"** — plus the `honeypot-*` data view (time field `timestamp`) they're built on are committed as a Saved Objects export at `kibana/saved_objects/honeypot-dashboards.ndjson`, and re-imported automatically by the `kibana_dashboards_setup` service on every `docker compose up` (`overwrite=true`, so it's idempotent and self-healing — a `down -v` purge no longer loses them). Kibana's default landing page is also set to the Web Traffic dashboard by that same service. To change a panel, edit it live in Kibana, then re-run `python3 kibana/build_dashboards.py` (rebuilds all 16 objects from source) or export just your change and commit the refreshed NDJSON — see `kibana/build_dashboards.py`'s docstring. Session-analysis / attack-pattern dashboards don't exist yet — that data currently lives only in the edge host's Redis, not here.
