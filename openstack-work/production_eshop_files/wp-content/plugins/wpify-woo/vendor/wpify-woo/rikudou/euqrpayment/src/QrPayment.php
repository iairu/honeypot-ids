...
namespace WpifyWooDeps\rikudou\EuQrPayment;

use DateTimeInterface;
use WpifyWooDeps\Endroid\QrCode\QrCode;
use WpifyWooDeps\JetBrains\PhpStorm\Deprecated;
use WpifyWooDeps\rikudou\EuQrPayment\Config\Configuration;
use WpifyWooDeps\rikudou\EuQrPayment\Config\ConfigurationInterface;
...
use WpifyWooDeps\rikudou\EuQrPayment\Helper\Utils;
...
final class QrPayment implements QrPaymentInterface
{
    ...
    public function getQrString(): string
    {
        ...
        if ($this->getAmount()) {
            $amount = $this->configuration->getAmountPrecision() !== null ? number_format($this->getAmount(), $this->configuration->getAmountPrecision(), '.', '') : $this->getAmount();
        } else {
            $amount = '';
        }
        $result[] = 'BCD';
