...
namespace WpifyWooDeps\rikudou\EuQrPayment\Tests;
...
use ReflectionClass;
use WpifyWooDeps\rikudou\EuQrPayment\Exceptions\InvalidIbanException;
use WpifyWooDeps\rikudou\EuQrPayment\Exceptions\InvalidOptionException;
use WpifyWooDeps\rikudou\EuQrPayment\Exceptions\UnsupportedMethodException;
use WpifyWooDeps\rikudou\EuQrPayment\Helper\ToStringIban;
use WpifyWooDeps\rikudou\EuQrPayment\Iban\IBAN;
use WpifyWooDeps\rikudou\EuQrPayment\Iban\IbanInterface;
use WpifyWooDeps\rikudou\EuQrPayment\Iban\Validator\ValidatorInterface;
use WpifyWooDeps\rikudou\EuQrPayment\QrPayment;
use WpifyWooDeps\rikudou\EuQrPayment\Sepa\CharacterSet;
use WpifyWooDeps\rikudou\EuQrPayment\Sepa\Purpose;
use Throwable;
class QrPaymentTest extends TestCase
{
    ...
    {
        $beneficiaries = [$this->getRandomString(10), $this->getRandomString(70), $this->getRandomString(71)];
    ...
    public function testSetPurpose()
    {
        $purposes = [
            Purpose::TRUST_FUND,
            // valid purpose constant
            'ABCD',
            // can be 0 - 4 characters long
            'ABC',
            'AB',
            'A',
        ];
        ...
        $this->expectException(InvalidArgumentException::class);
        $this->getDefaultPayment()->setPurpose('ABCDE');
        // 5 characters long
    }
    public function testGetQrString()
    {
        $payment = $this->getDefaultPayment();
        $payment->setPurpose(Purpose::ACCOUNT_MANAGEMENT)->setBeneficiaryName('My Company')->setSwift('AIRACZPP')->setAmount(10)->setCurrency('EUR')->setComment('Random comment')->setRemittanceText('Invoice ID: 1')->setCharacterSet(CharacterSet::UTF_8);
        $this->assertEquals("BCD\n002\n1\nSCT\nAIRACZPP\nMy Company\nCZ5530300000001325090010\nEUR10\nACCT\n\nInvoice ID: 1\nRandom comment", $payment->getQrString());
        $payment = $this->getDefaultPayment();
        $payment->setPurpose(Purpose::ACCOUNT_MANAGEMENT)->setBeneficiaryName('My Company')->setSwift('AIRACZPP')->setAmount(10)->setCurrency('EUR')->setComment('Random comment')->setCreditorReference('RF0000000')->setCharacterSet(CharacterSet::UTF_8);
        $this->assertEquals("BCD\n002\n1\nSCT\nAIRACZPP\nMy Company\nCZ5530300000001325090010\nEUR10\nACCT\nRF0000000\n\nRandom comment", $payment->getQrString());
        $payment = $this->getDefaultPayment();
        ...
        $payment = $this->getDefaultPayment();
        $payment->setBeneficiaryName('My Company')->setRemittanceText('test')->setCreditorReference('RF123');
        $payment->getQrString();
    }
    ...
    public function testGetQrStringInvalidIban()
    {
        ...
        (new QrPayment(new IBAN('')))->setBeneficiaryName('random name')->getQrString();
    }
    /**
    ...
    public function testGetQrStringTooLong()
    {
        $this->expectException(LogicException::class);
        $payment = $this->getDefaultPayment();
        $payment->setSwift($this->getRandomString(11))->setBeneficiaryName($this->getRandomString(70))->setAmount(999999999.99)->setPurpose($this->getRandomString(4))->setRemittanceText($this->getRandomString(140))->setComment($this->getRandomString(70));
        $payment->getQrString();
    }
    public function testSetInformation()
    {
        $strings = [$this->getRandomString(10), $this->getRandomString(70), $this->getRandomString(71)];
    ...
    {
        $strings = [$this->getRandomString(10), $this->getRandomString(140), $this->getRandomString(141)];
    ...
    {
        $shortBics = ['A', 'AB', 'ABC', 'ABCD', 'ABCDE', 'ABCDEF', 'ABCDEFG'];
        $validLengthBics = ['ABCDEFGH', 'ABCDEFGHI', 'ABCDEFGHIJ', 'ABCDEFGHIJK'];
        $tooLongBics = ['ABCDEFGHIJKL', 'ABCDEFGHIJKLM', 'ABCDEFGHIJKLMN', 'ABCDEFGHIJKLMNO'];
    ...
    public function testSetOptions()
    {
        ...
        $options = ['characterSet' => CharacterSet::ISO_8859_4, 'bic' => 'testtest', 'beneficiaryName' => 'Random Guy', 'amount' => 150.34, 'purpose' => Purpose::ACCOUNT_OVERDRAFT_REPAYMENT, 'remittanceText' => 'Some string', 'creditorReference' => 'Some creditor', 'information' => 'Another string', 'currency' => 'USD'];
    ...
    public function testSetOptionsUnwritableProperty()
    {
        ...
        $this->getDefaultPayment()->setOptions(['iban' => 'CZ5530300000001325090010']);
    }
    public function testSetOptionsNonexistentProperty()
    {
        $this->expectException(InvalidOptionException::class);
        $this->getDefaultPayment()->setOptions(['someRandomOption' => 'someValue']);
    }
    public function testSetCreditorReference()
    {
        $strings = [$this->getRandomString(10), $this->getRandomString(35), $this->getRandomString(36)];
