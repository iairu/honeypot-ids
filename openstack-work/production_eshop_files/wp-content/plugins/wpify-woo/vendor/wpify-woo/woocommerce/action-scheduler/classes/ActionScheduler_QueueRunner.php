...
class ActionScheduler_QueueRunner extends \ActionScheduler_Abstract_QueueRunner
{
    ...
    public function init()
    {
        ...
        if (!\wp_next_scheduled(self::WP_CRON_HOOK, $cron_context)) {
            ...
            \wp_schedule_event(\time(), $schedule, self::WP_CRON_HOOK, $cron_context);
        }
    ...
    public function maybe_dispatch_async_request()
    {
        ...
        if (\is_admin() && !\ActionScheduler::lock()->is_locked('async-request-runner') && \ActionScheduler::lock()->set('async-request-runner')) {
    ...
    public function run($context = 'WP Cron')
    {
        ...
        if (\false === $this->has_maximum_concurrent_batches()) {
            ...
            } while ($processed_actions_in_batch > 0 && !$this->batch_limits_exceeded($this->processed_actions_count));
    ...
    protected function do_batch($size = 100, $context = '')
    {
        ...
        foreach ($claim->get_actions() as $action_id) {
            ...
            if (!\in_array($action_id, $this->store->find_actions_by_claim_id($claim->get_id()), \true)) {
                break;
            }
    ...
    protected function clear_caches()
    {
        ...
        } elseif (!\wp_using_ext_object_cache() || \apply_filters('action_scheduler_queue_runner_flush_cache', \false)) {
            \wp_cache_flush();
        }
    }
    /**
 ...
 * Class ActionScheduler_QueueRunner
 */
\class_alias('WpifyWooDeps\ActionScheduler_QueueRunner', 'ActionScheduler_QueueRunner', \false);
