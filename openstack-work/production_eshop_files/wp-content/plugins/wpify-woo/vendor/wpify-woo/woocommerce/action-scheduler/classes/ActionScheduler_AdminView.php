...
class ActionScheduler_AdminView extends \ActionScheduler_AdminView_Deprecated
{
    ...
    {
        $hook_suffix = \add_submenu_page('tools.php', \__('Scheduled Actions', 'action-scheduler'), \__('Scheduled Actions', 'action-scheduler'), 'manage_options', 'action-scheduler', array($this, 'render_admin_ui'));
    ...
    protected function check_pastdue_actions()
    {
        ...
        $query_args = array('date' => \as_get_datetime_object(\time() - $threshold_seconds), 'status' => \ActionScheduler_Store::STATUS_PENDING, 'per_page' => $threshold_min);
        ...
        if (!\boolval($check)) {
            ...
            \set_transient('action_scheduler_last_pastdue_actions_check', \time(), $interval);
            return;
        }
        $actions_url = \add_query_arg(array('page' => 'action-scheduler', 'status' => 'past-due', 'order' => 'asc'), \admin_url('tools.php'));
        // Print notice.
        echo '<div class="notice notice-warning"><p>';
        \printf(\wp_kses(
            // translators: 1) is the number of affected actions, 2) is a link to an admin screen.
            \_n('<strong>Action Scheduler:</strong> %1$d <a href="%2$s">past-due action</a> found; something may be wrong. <a href="https://actionscheduler.org/faq/#my-site-has-past-due-actions-what-can-i-do" target="_blank">Read documentation &raquo;</a>', '<strong>Action Scheduler:</strong> %1$d <a href="%2$s">past-due actions</a> found; something may be wrong. <a href="https://actionscheduler.org/faq/#my-site-has-past-due-actions-what-can-i-do" target="_blank">Read documentation &raquo;</a>', $num_pastdue_actions, 'action-scheduler'),
            array('strong' => array(), 'a' => array('href' => \true, 'target' => \true))
        ), \absint($num_pastdue_actions), \esc_attr(\esc_url($actions_url)));
        echo '</p></div>';
 ...
 * @codeCoverageIgnore
 */
\class_alias('WpifyWooDeps\ActionScheduler_AdminView', 'ActionScheduler_AdminView', \false);
