...
class Get_Command extends \ActionScheduler_WPCLI_Command
{
    ...
    public function execute()
    {
        ...
        if (is_a($action, \Action_Scheduler\WP_CLI\Action\ActionScheduler_NullAction::class)) {
        ...
        if ($only_logs) {
            $args = array('format' => \WP_CLI\Utils\get_flag_value($this->assoc_args, 'format', 'table'));
        ...
        $action_arr = array('id' => $this->args[0], 'hook' => $action->get_hook(), 'status' => $status, 'args' => $action->get_args(), 'group' => $action->get_group(), 'recurring' => $action->get_schedule()->is_recurring() ? 'yes' : 'no', 'scheduled_date' => $this->get_schedule_display_string($action->get_schedule()), 'log_entries' => $log_entries);
