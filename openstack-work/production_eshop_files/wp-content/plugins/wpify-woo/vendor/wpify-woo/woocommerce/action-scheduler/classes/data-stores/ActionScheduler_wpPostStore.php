...
class ActionScheduler_wpPostStore extends \ActionScheduler_Store
{
    ...
    {
        $post = array('post_type' => self::POST_TYPE, 'post_title' => $action->get_hook(), 'post_content' => \wp_json_encode($action->get_args()), 'post_status' => $action->is_finished() ? 'publish' : 'pending', 'post_date_gmt' => $this->get_scheduled_date_string($action, $scheduled_date), 'post_date' => $this->get_scheduled_date_string_local($action, $scheduled_date));
        return $post;
    }
    /**
     * Save post array.
    ...
    public function set_unique_post_slug($override_slug, $slug, $post_ID, $post_status, $post_type)
    {
        if (self::POST_TYPE === $post_type) {
            $override_slug = \uniqid(self::POST_TYPE . '-', \true) . '-' . \wp_generate_password(32, \false);
        }
    ...
    protected function make_action_from_post($post)
    {
        ...
        $schedule = \get_post_meta($post->ID, self::SCHEDULE_META_KEY, \true);
    ...
    public function get_claim_count()
    {
        ...
        return $wpdb->get_var($wpdb->prepare("SELECT COUNT(DISTINCT post_password) FROM {$wpdb->posts} WHERE post_password != '' AND post_type = %s AND post_status IN ('in-progress','pending')", array(self::POST_TYPE)));
    }
    /**
     * Generate claim id.
     *
    ...
    {
        $claim_id = \md5(\microtime(\true) . \wp_rand(0, 1000));
    ...
    protected function claim_actions($claim_id, $limit, ?\DateTime $before_date = null, $hooks = array(), $group = '')
    {
        ...
        $params = array($claim_id, \current_time('mysql', \true), \current_time('mysql'));
    ...
    protected function get_actions_by_group($group, $limit, \DateTime $date)
    {
        ...
        if (!\term_exists($group, self::GROUP_TAXONOMY)) {
        ...
        $query_args = array(
            ...
            'date_query' => array('column' => 'post_date_gmt', 'before' => $date->format('Y-m-d H:i'), 'inclusive' => \true),
            'tax_query' => array(
                // phpcs:ignore WordPress.DB.SlowDBQuery
                array('taxonomy' => self::GROUP_TAXONOMY, 'field' => 'slug', 'terms' => $group, 'include_children' => \false),
            ),
        );
    ...
    public function find_actions_by_claim_id($claim_id)
    {
        ...
        // phpcs:ignore WordPress.DB.DirectDatabaseQuery.DirectQuery, WordPress.DB.DirectDatabaseQuery.NoCaching
        $results = $wpdb->get_results($wpdb->prepare("SELECT ID, post_date_gmt FROM {$wpdb->posts} WHERE post_type = %s AND post_password = %s", array(self::POST_TYPE, $claim_id)));
    ...
    public function release_claim(\ActionScheduler_ActionClaim $claim)
    {
        ...
        // phpcs:ignore WordPress.DB.DirectDatabaseQuery.DirectQuery, WordPress.DB.DirectDatabaseQuery.NoCaching
        $action_ids = $wpdb->get_col($wpdb->prepare("SELECT ID, post_date_gmt FROM {$wpdb->posts} WHERE post_type = %s AND post_password = %s AND post_status = %s", self::POST_TYPE, $claim_id, self::STATUS_PENDING));
        if (empty($action_ids)) {
            return;
            // nothing to do.
        }
    ...
    public function unclaim_action($action_id)
    {
        ...
        global $wpdb;
        //phpcs:ignore WordPress.DB.DirectDatabaseQuery.DirectQuery, WordPress.DB.DirectDatabaseQuery.NoCaching
        $result = $wpdb->query($wpdb->prepare("UPDATE {$wpdb->posts} SET post_password = '' WHERE ID = %d AND post_type = %s", $action_id, self::POST_TYPE));
        if (\false === $result) {
    ...
    public function mark_failure($action_id)
    {
        ...
        global $wpdb;
        // phpcs:ignore WordPress.DB.DirectDatabaseQuery.DirectQuery, WordPress.DB.DirectDatabaseQuery.NoCaching
        $result = $wpdb->query($wpdb->prepare("UPDATE {$wpdb->posts} SET post_status = %s WHERE ID = %d AND post_type = %s", self::STATUS_FAILED, $action_id, self::POST_TYPE));
        if (\false === $result) {
            /* translators: %s: action ID */
    ...
    public function log_execution($action_id)
    {
        ...
        global $wpdb;
        // phpcs:ignore WordPress.DB.DirectDatabaseQuery.DirectQuery, WordPress.DB.DirectDatabaseQuery.NoCaching
        $status_updated = $wpdb->query($wpdb->prepare("UPDATE {$wpdb->posts} SET menu_order = menu_order+1, post_status=%s, post_modified_gmt = %s, post_modified = %s WHERE ID = %d AND post_type = %s", self::STATUS_RUNNING, \current_time('mysql', \true), \current_time('mysql'), $action_id, self::POST_TYPE));
        if (!$status_updated) {
    ...
    public function mark_complete($action_id)
    {
        ...
        \add_filter('wp_insert_post_data', array($this, 'filter_insert_post_data'), 10, 1);
        \add_filter('pre_wp_unique_post_slug', array($this, 'set_unique_post_slug'), 10, 5);
        $result = \wp_update_post(array('ID' => $action_id, 'post_status' => 'publish'), \true);
        \remove_filter('wp_insert_post_data', array($this, 'filter_insert_post_data'), 10);
        \remove_filter('pre_wp_unique_post_slug', array($this, 'set_unique_post_slug'), 10);
 ...
 * Class ActionScheduler_wpPostStore
 */
\class_alias('WpifyWooDeps\ActionScheduler_wpPostStore', 'ActionScheduler_wpPostStore', \false);
