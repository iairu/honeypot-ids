...
class ActionScheduler_QueueCleaner
{
    ...
    public function clean_actions(array $statuses_to_purge, \DateTime $cutoff_date, $batch_size = null, $context = 'old')
    {
        ...
        foreach ($statuses_to_purge as $status) {
            $actions_to_delete = $this->store->query_actions(array('status' => $status, 'modified' => $cutoff, 'modified_compare' => '<=', 'per_page' => $batch_size, 'orderby' => 'none'));
    ...
    public function reset_timeouts($time_limit = 300)
    {
        ...
        $actions_to_reset = $this->store->query_actions(array('status' => \ActionScheduler_Store::STATUS_PENDING, 'modified' => $cutoff, 'modified_compare' => '<=', 'claimed' => \true, 'per_page' => $this->get_batch_size(), 'orderby' => 'none'));
 ...
 * Class ActionScheduler_QueueCleaner
 */
\class_alias('WpifyWooDeps\ActionScheduler_QueueCleaner', 'ActionScheduler_QueueCleaner', \false);
