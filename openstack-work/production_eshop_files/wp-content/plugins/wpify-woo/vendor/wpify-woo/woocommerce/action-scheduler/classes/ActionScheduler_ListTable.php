...
class ActionScheduler_ListTable extends \ActionScheduler_Abstract_ListTable
{
    ...
    public function __construct(\ActionScheduler_Store $store, \ActionScheduler_Logger $logger, \ActionScheduler_QueueRunner $runner)
    {
        ...
        $this->columns = array('hook' => \__('Hook', 'action-scheduler'), 'status' => \__('Status', 'action-scheduler'), 'args' => \__('Arguments', 'action-scheduler'), 'group' => \__('Group', 'action-scheduler'), 'recurrence' => \__('Recurrence', 'action-scheduler'), 'schedule' => \__('Scheduled Date', 'action-scheduler'), 'log_entries' => \__('Log', 'action-scheduler'));
        $this->sort_by = array('schedule', 'hook', 'group');
        $this->search_by = array('hook', 'args', 'claim_id');
        $request_status = $this->get_request_status();
        if (empty($request_status)) {
            $this->sort_by[] = 'status';
        } elseif (\in_array($request_status, array('in-progress', 'failed'), \true)) {
        ...
        \add_screen_option('per_page', array('default' => $this->items_per_page));
    ...
    private static function human_interval($interval, $periods_to_include = 2)
    {
        ...
        for ($time_period_index = 0, $periods_included = 0, $seconds_remaining = $interval; $time_period_index < $num_time_periods && $seconds_remaining > 0 && $periods_included < $periods_to_include; $time_period_index++) {
            ...
            if ($periods_in_interval > 0) {
                ...
                $output .= \sprintf(\translate_nooped_plural(self::$time_periods[$time_period_index]['names'], $periods_in_interval, 'action-scheduler'), $periods_in_interval);
    ...
    public function column_args(array $row)
    {
        ...
        foreach ($row['args'] as $key => $value) {
            $row_html .= \sprintf('<li><code>%s => %s</code></li>', \esc_html(\var_export($key, \true)), \esc_html(\var_export($value, \true)));
    ...
    {
        global $wpdb;
        if ((\is_a($this->store, 'ActionScheduler_HybridStore') || \is_a($this->store, 'ActionScheduler_DBStore')) && \apply_filters('action_scheduler_enable_recreate_data_store', \true)) {
            ...
            foreach ($table_list as $table_name) {
                if (!\in_array($wpdb->prefix . $table_name, $found_tables, \true)) {
                    $this->admin_notices[] = array('class' => 'error', 'message' => \__('It appears one or more database tables were missing. Attempting to re-create the missing table(s).', 'action-scheduler'));
        ...
        if ($this->runner->has_maximum_concurrent_batches()) {
            ...
            if (\false === $async_request_lock_expiration || $async_request_lock_expiration < \time()) {
                ...
                $async_request_message = \sprintf(\__('The next queue will begin processing in approximately %d seconds.', 'action-scheduler'), $async_request_lock_expiration - \time());
            }
 ...
 * @codeCoverageIgnore
 */
\class_alias('WpifyWooDeps\ActionScheduler_ListTable', 'ActionScheduler_ListTable', \false);
