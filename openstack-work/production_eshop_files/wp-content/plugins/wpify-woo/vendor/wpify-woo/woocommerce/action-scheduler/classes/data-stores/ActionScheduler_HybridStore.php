...
class ActionScheduler_HybridStore extends Store
{
    ...
    public function set_autoincrement($table_name, $table_suffix)
    {
        if (\ActionScheduler_StoreSchema::ACTIONS_TABLE === $table_suffix) {
            ...
            $row_count = $wpdb->insert($wpdb->{\ActionScheduler_StoreSchema::ACTIONS_TABLE}, array('action_id' => $this->demarkation_id, 'hook' => '', 'status' => '', 'scheduled_date_gmt' => $date_gmt, 'scheduled_date_local' => $date_local, 'last_attempt_gmt' => $date_gmt, 'last_attempt_local' => $date_local));
            if ($row_count > 0) {
                $wpdb->delete($wpdb->{\ActionScheduler_StoreSchema::ACTIONS_TABLE}, array('action_id' => $this->demarkation_id));
            }
        }
    }
    /**
    ...
    {
        if ($primary_first) {
            $stores = array($this->primary_store, $this->secondary_store);
        } elseif ($action_id < $this->demarkation_id) {
            $stores = array($this->secondary_store, $this->primary_store);
        } else {
            $stores = array($this->primary_store);
        }
 ...
 * @since 3.0.0
 */
\class_alias('WpifyWooDeps\ActionScheduler_HybridStore', 'ActionScheduler_HybridStore', \false);
