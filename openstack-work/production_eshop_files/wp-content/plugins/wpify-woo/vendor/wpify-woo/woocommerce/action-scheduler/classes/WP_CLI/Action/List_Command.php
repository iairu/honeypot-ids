...
class List_Command extends \ActionScheduler_WPCLI_Command
{
    ...
    public function execute()
    {
        ...
        $fields = array('id', 'hook', 'status', 'group', 'recurring', 'scheduled_date');
        ...
        if (in_array($formatter->format, array('ids', 'count'), \true)) {
            $return_format = '\'ids\'';
        }
        // phpcs:ignore WordPress.PHP.DevelopmentFunctions.error_log_var_export
        $params = var_export($query_args, \true);
        if (empty($query_args)) {
            $params = 'array()';
        }
        ...
        switch ($formatter->format) {
            ...
            default:
                ...
                foreach ($actions as $action_id => $action) {
                    $action_arr = array('id' => $action_id, 'hook' => $action->get_hook(), 'status' => $store->get_status($action_id), 'args' => $action->get_args(), 'group' => $action->get_group(), 'recurring' => $action->get_schedule()->is_recurring() ? 'yes' : 'no', 'scheduled_date' => $this->get_schedule_display_string($action->get_schedule()), 'log_entries' => array());
                    foreach ($logger->get_logs($action_id) as $log_entry) {
                        $action_arr['log_entries'][] = array('date' => $log_entry->get_date()->format(static::DATE_FORMAT), 'message' => $log_entry->get_message());
                    }
