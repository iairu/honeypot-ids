    ...
    public function save_action(\ActionScheduler_Action $action, ?\DateTime $scheduled_date = null, ?\DateTime $last_attempt_date = null)
    {
        try {
            ...
            if (null !== $last_attempt_date) {
                $data = array('last_attempt_gmt' => $this->get_scheduled_date_string($action, $last_attempt_date), 'last_attempt_local' => $this->get_scheduled_date_string_local($action, $last_attempt_date));
 ...
 * @since 3.0.0
 */
\class_alias('WpifyWooDeps\ActionScheduler_DBStoreMigrator', 'ActionScheduler_DBStoreMigrator', \false);
