...
abstract class ActionScheduler_Abstract_ListTable extends \WP_List_Table
{
    ...
    protected function process_bulk_action()
    {
        ...
        if (isset($_SERVER['REQUEST_URI'])) {
            \wp_safe_redirect(\remove_query_arg(array('_wp_http_referer', '_wpnonce', 'ID', 'action', 'action2'), \esc_url_raw(\wp_unslash($_SERVER['REQUEST_URI']))));
            exit;
        }
    }
    /**
    ...
    {
        return \array_merge($this->sort_by, array('page', 'status', 'tab'));
    }
    /**
    ...
    protected function get_request_orderby()
    {
        ...
        if (!empty($_GET['orderby']) && \in_array($_GET['orderby'], $valid_sortable_columns, \true)) {
    ...
    protected function get_table_columns()
    {
        ...
        if (!\in_array($this->ID, $columns, \true)) {
            $columns[] = $this->ID;
        }
        return $columns;
    }
    ...
    protected function get_items_query_search()
    {
        ...
        foreach ($this->search_by as $column) {
            ...
            // phpcs:ignore WordPress.Security.NonceVerification.Recommended, WordPress.DB.PreparedSQL.NotPrepared
        }
    ...
    protected function get_items_query_filters()
    {
        ...
        foreach ($this->filter_by as $column => $options) {
            ...
            //phpcs:ignore WordPress.Security.NonceVerification.Recommended, WordPress.DB.PreparedSQL.InterpolatedNotPrepared
        }
    ...
    protected function process_row_actions()
    {
        ...
        if (isset($_SERVER['REQUEST_URI'])) {
            \wp_safe_redirect(\remove_query_arg(array('row_id', 'row_action', 'nonce'), \esc_url_raw(\wp_unslash($_SERVER['REQUEST_URI']))));
            exit;
        }
    }
    /**
 ...
 * @since  2.0.0
 */
\class_alias('WpifyWooDeps\ActionScheduler_Abstract_ListTable', 'ActionScheduler_Abstract_ListTable', \false);
