...
class SiteOptions extends OptionsIntegration
{
    ...
    public function __construct(array $args, CustomFields $custom_fields)
    {
        ...
        $this->submit_button = $args['submit_button'] ?? \true;
        ...
        $this->id = sanitize_title(join('-', array('site-options', $this->blog_id, $this->menu_slug)));
    ...
    public function register(): void
    {
        ...
        $hook_suffix = add_submenu_page('', $this->page_title, $this->menu_title, $this->capability, $this->menu_slug, array($this, 'render'));
    ...
    public function render(): void
    {
        ...
        ?>"><?php 
        echo esc_html(__('Visit', 'wpify-custom-fields'));
        ?></a>
				|
				<a href="<?php 
        echo esc_url(get_admin_url($id));
        ?>"><?php 
        echo esc_html(__('Dashboard', 'wpify-custom-fields'));
        ?></a>
			</p>
			<?php 
        ...
        ?>" method="POST" name="form" data-context="site-options">
        ...
        if (isset($_GET['updated']) && isset($_GET['page']) && $this->menu_slug === $_GET['page']) {
            ?>
					<div id="message" class="updated notice is-dismissible">
						<p><?php 
            echo esc_html($this->success_message);
            ?></p>
					</div>
					<?php 
        }
    ...
    public function render_help(): void
    {
        foreach ($this->help_tabs as $key => $tab) {
            $tab = wp_parse_args($tab, array('id' => '', 'title' => '', 'content' => ''));
            if (empty($tab['id'])) {
    ...
    public function register_settings(): void
    {
        ...
        if (empty($this->option_name)) {
            ...
            register_setting($this->option_group, $this->option_name, array('type' => 'object', 'label' => $this->page_title, 'sanitize_callback' => $this->custom_fields->sanitize_option_value($items), 'show_in_rest' => \false, 'default' => array()));
        }
    }
    /**
