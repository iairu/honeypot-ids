 ...
 */
class CouponOptions extends ItemsIntegration
{
    /**
    ...
    public function __construct(array $args, CustomFields $custom_fields)
    {
        ...
        $this->tab = $tab;
        $this->id = sanitize_title(join('-', array('product-options', $this->tab['id'], sanitize_title($this->tab['label']), $this->hook_priority)));
     ...
     */
    public function woocommerce_coupon_data_tabs(array $tabs): array
    {
    ...
    /**
     * Retrieves the WooCommerce coupon based on the item's ID.
    ...
    public function register_meta(): void
    {
        ...
        foreach ($items as $item) {
            register_post_meta('coupon', $item['id'], array('type' => $this->custom_fields->get_wp_type($item), 'description' => $item['label'], 'single' => \true, 'default' => $this->custom_fields->get_default_value($item), 'sanitize_callback' => $this->custom_fields->sanitize_item_value($item), 'show_in_rest' => \false));
        }
    }
    /**
    ...
    {
        return $this->get_coupon()->get_meta($name) ?? $default_value;
    }
    /**
