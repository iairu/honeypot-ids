...
class Api
{
     ...
     * @return void
     */
    public function __construct(private readonly CustomFields $custom_fields, private readonly Helpers $helpers)
    {
    ...
    {
        $this->register_rest_route('url-title', WP_REST_Server::READABLE, fn(WP_REST_Request $request) => $this->helpers->get_url_title($request->get_param('url')), array('url' => array('required' => \true)), array($this, 'cap_edit_posts'));
        $this->register_rest_route('posts', WP_REST_Server::READABLE, fn(WP_REST_Request $request) => $this->helpers->get_posts($request->get_params()), array('post_type' => array('required' => \true)), array($this, 'cap_edit_posts'));
        $this->register_rest_route('terms', WP_REST_Server::READABLE, fn(WP_REST_Request $request) => $this->helpers->get_terms($request->get_params()), array('taxonomy' => array('required' => \true)), array($this, 'cap_edit_posts'));
        $this->register_rest_route('mapycz-api-key', WP_REST_Server::EDITABLE, fn(WP_REST_Request $request) => update_option('mapy_cz_api_key', $request->get_param('api_key')), array('api_key' => array('required' => \true)), array($this, 'cap_manage_options'));
        $this->register_rest_route('mapycz-api-key', WP_REST_Server::READABLE, fn() => get_option('mapy_cz_api_key'), array(), array($this, 'cap_edit_posts'));
        ...
        $this->register_rest_route('direct-file-info', WP_REST_Server::READABLE, array($this, 'handle_direct_file_info'), array('file_path' => array('required' => \true)), array($this, 'cap_edit_posts'));
        $this->register_rest_route('cloudflare/zones', WP_REST_Server::CREATABLE, array($this, 'handle_cloudflare_zones'), array('email' => array('required' => \true), 'api_key' => array('required' => \true), 'page' => array('required' => \false)), array($this, 'cap_manage_options'));
    }
    /**
    ...
    public function handle_direct_file_upload()
    {
        ...
        try {
            $result = wp_handle_upload(
                $_FILES['file'],
                // phpcs:ignore
                array('test_form' => \false, 'mimes' => null)
            );
        } finally {
        ...
        return array('temp_path' => $result['file'], 'filename' => basename($result['file']), 'size' => $file['size'], 'type' => $result['type']);
    }
    /**
    ...
    public function handle_direct_file_info(WP_REST_Request $request)
    {
        ...
        return array('size' => $filesize, 'type' => $filetype['type'], 'filename' => basename($real));
    }
    /**
