...
class Metabox extends ItemsIntegration
{
    ...
    public function __construct(array $args, CustomFields $custom_fields)
    {
        ...
        $this->id = $args['id'] ?? sanitize_title(join('_', array($this->title, $this->screen, $this->context, $this->priority)));
    ...
    {
        if (in_array($post_type, $this->post_types, \true)) {
            add_meta_box($this->id, $this->title, array($this, 'render'), $post_type, $this->context, $this->priority, $this->callback_args);
        }
    }
    /**
    ...
    {
        return get_post_meta($this->get_item_id(), $name, \true) ?? $default_value;
    }
    /**
