...
class ProductVariationOptions extends ItemsIntegration
{
    ...
    public function __construct(array $args, CustomFields $custom_fields)
    {
        ...
        $this->id = sanitize_title(join('-', array('variation-options', $this->tab['id'], sanitize_title($this->tab['label']), $this->hook_priority)));
        if (!defined('WpifyWooDeps\WP_CLI') || \false === WP_CLI) {
            if (in_array($this->after, array('pricing', 'inventory', 'dimensions', 'download'), \true)) {
