...
##############################################
# Phase orchestrator for OpenStack-work
# Target host: 'arch'. Requires passwordless SSH.
# Remote project path: /mnt/hgfs/openstack-work
##############################################

SSH_HOST="${SSH_HOST:-arch}"
SSH_USER="${SSH_USER:-root}"
REMOTE_BASE="${REMOTE_BASE:-/mnt/hgfs/openstack-work}"
...
ssh -o BatchMode=yes "${SSH_USER}@${SSH_HOST}" bash -lc "set -e; cd '${REMOTE_BASE}';\n  echo 'Suricata logs (fast.log/eve.json) head:'; if [ -f '/mnt/hgfs/openstack-work/suricata_logs/fast.log' ]; then tail -n 50 /mnt/hgfs/openstack-work/suricata_logs/fast.log; fi;\n  if [ -f '/mnt/hgfs/openstack-work/suricata_logs/eve.json' ]; then tail -n 50 /mnt/hgfs/openstack-work/suricata_logs/eve.json; fi;\n  echo 'Dead code markers in repo:'; grep -R --color=never -n "DEAD CODE" . || true" || true
...
# OpenStack-work Phase Walkthrough (Preview)
