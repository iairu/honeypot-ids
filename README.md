...
- **Dashboard: automatic per-service error badge on the Health diagram**: a red "!N" badge appears in a node's bottom-right corner the moment any log line for that service contains "error" (case-insensitive), N counting up live. `ui/error_monitor.py` taps the Services page's existing always-running combined log tail (no extra `docker compose logs` process spun up), parsing `docker compose logs`' own `<service>-<replica> | message` line prefix; counts reset per-target whenever that target's tail restarts, so the same `--tail=50` scrollback isn't recounted on every Start/Restart. Shared between the Services and Health pages via one `ErrorLogMonitor` instance (`ui/main_window.py`), so it keeps counting regardless of which page is currently open. Matching excludes "error" found only inside an identifier/file path (`nginx_error_in`, `error.log`) -- confirmed live these otherwise false-triggered on Vector's own routine startup log line.
- **Real errors the badge above surfaced, fixed at the source (this session)**:
  - `honeypot_content_sync`'s `mysqldump` was failing on **every single replication cycle** with `Access denied; you need ... PROCESS privilege(s) ... when trying to dump tablespaces` -- `production_user` was never granted (and doesn't need) that privilege for anything else this script does. Fixed by adding `--no-tablespaces` to `prod_mysqldump()` (`scripts/replicate_content_to_honeypot.sh`), the standard fix for this well-known mysqldump behavior, rather than granting a broader privilege.
  - `honeypot_db_migration` produced a wall of `ERROR 2003: Can't connect to MySQL server ... (111)` under load (confirmed during this session's own `.env`-triggered recreate cascade, see below) -- its `sleep 5` "wait for pools to settle" was a blind, fixed delay, not an actual readiness check. Replaced with a real per-host TCP-connect wait loop (deliberately a raw TCP probe, not a MySQL auth check -- pool 2/3 may already have had their password rotated by a prior run, which a `mysql -e 'SELECT 1'`-based check would have misread as "still down").
  - `honeypot_db_migration`'s `apply_migrations()` never actually exited non-zero on a real migration failure despite its own comment claiming it did ("Exits with status 1 if any migration fails") -- confirmed live it always printed "completed successfully" regardless. Fixed to track real failures and exit 1, while keeping `01_clean-honeypot-data.sql`'s own documented best-effort DELETEs (expected to fail with "table doesn't exist" on a freshly-copied pool, before `honeypot_db_seed_N`'s `wp core install` has run) non-fatal and its stderr silenced. This is what made `02_pool_tracking.sql`'s own `wp_options` migration-marker `INSERT` -- previously ALSO always failing on a fresh pool for the same reason, just undocumented -- worth guarding instead of masking: it's now wrapped in an `information_schema` check + `PREPARE`/`EXECUTE` so it only runs once `wp_options` actually exists, and the file completes cleanly (exit 0) either way.
...
- **Dashboard: "Collapse repeated lines" on the Log Search page** (on by default): folds a run of consecutive, byte-identical matches into one line with a "(×N)" suffix (`uniq -c` semantics -- a recurrence with other lines in between is left alone), so a crash-looping container or a job failing the same way every cycle doesn't bury everything else in the result list. Toggling it re-renders the already-fetched results instantly rather than re-running the search.
- **`honeypot_content_sync`'s exploit-gating Redis flag had never actually worked.** `redis_set_replicating()`/`redis_clear_replicating()` (`scripts/replicate_content_to_honeypot.sh`) passed `--no-auth-warning` to `redis-cli` -- a flag added in Redis 5.0 (2018) that the container's `yum install redis` (an EL7-image default-repo package) installs as redis-cli **3.2.12**, from 2016, which doesn't recognize it. Confirmed live: an unrecognized flag is a *fatal* parse error for redis-cli, not a warning, so every single `SET`/`DEL` of `honeypot_pool_replicating:<N>` had been silently failing on every sync cycle since this flag was added -- meaning `pool_router.lua`'s failover never actually saw a pool as "mid-replication" and routed around it (§8's roadmap "Exploit gating" note was describing intended, not actual, behavior). Fixed by dropping the flag (the warning it suppresses is redirected to `/dev/null` instead, which works on any redis-cli version); verified live, both functions now succeed with no fallback-log lines on every cycle.
- **Dashboard: a distinct "created" (never started) status.** Diagnosed a genuine `Access denied for user 'production_user'@'localhost'` log burst back to its real cause: 13 containers -- effectively the whole application layer (`reverse_proxy`, `vector`, all 3 eshops, `production_eshop`, both seed/migration jobs, `backup_service`, `content_sync`) -- were stuck in `docker compose`'s `created` state, never actually started (an interrupted `up` leaves whatever hasn't started yet sitting there indefinitely; it does not self-heal). `production_database` had consequently bootstrapped from a genuinely empty volume with no `wp_*` schema at all, and the healthcheck's transient "Access denied" during that fresh-init window was just a symptom, not the real problem. `classify()`/`summarize_status()` (`ui/health_diagram.py`, `ui/page_services.py`) previously lumped `State=created` in with "down," making this exact situation visually indistinguishable from a normal "not started yet" -- now it gets its own color/label ("N created but never started -- re-run Start") on both the Health diagram and the Services page. Fixed live by re-running `docker compose up -d` for the whole stack; confirmed the site is fully restored (real content, `200 OK`).
- **Diagnosed (not a bug) — sporadic `reverse_proxy` "SSL_do_handshake() failed ... certificate unknown"**: traced to the dashboard's own embedded Exploits-page browser (`ps`/network inspection identified the exact PID; `reverse_proxy`'s access log at the SAME timestamp shows a `QtWebEngine/6.11.1` User-Agent successfully loading dozens of WooCommerce/Elementor assets with `200`s) -- a stray parallel/speculative connection among many, not a real problem; nginx itself logs it at `[info]`, not `[error]`. It IS a real gap in `ui/browser_widget.py`'s per-page `certificateError` handler though (confirmed it doesn't cover every connection type Chromium's network stack can open), so `dashboard/main.py` now sets `QTWEBENGINE_CHROMIUM_FLAGS=--ignore-certificate-errors` before the engine initializes -- a supplement, not a new category of risk (the existing handler already accepted every cert error unconditionally, regardless of origin). Takes effect on the dashboard's next restart. The one genuinely-transient `session_store` Redis timeout in the same log batch: confirmed only 1 occurrence in 2 hours, `session_store` healthy since -- same one-off host-load pattern as this session's other transient findings, not a recurring issue.
...
- **Full purge + fresh `docker compose up` re-verified clean, and one real gap found and fixed.** Purged every `ids` container/volume and brought the stack up from nothing while monitoring the combined log output live for anything containing "error". Every earlier fix this session held (zero recurrences of the `mysqldump`/`redis-cli`/`ERROR 2003`/migration-`[FAILED]` patterns); the only log lines seen were the already-diagnosed-benign `[Note]` MySQL startup noise and one already-diagnosed transient `wp_options` race, both self-resolving. One real, actionable gap did surface: `docker compose up` itself aborted mid-chain with `production_database`/`honeypot_database_3` reported "unhealthy" (leaving every downstream container stuck in `created`, needing a manual second `up -d`) — `start_period: 240s` on the 4 database healthchecks was sized for ONE database bootstrapping from an empty volume in isolation (an earlier session's own fix, see its comment), but a purge always bootstraps all 4 simultaneously, and confirmed live the resulting I/O contention is severe (`InnoDB: page_cleaner: 1000ms intended loop took 40733ms`). Bumped to `420s`; verified the rendered config via `docker compose config`, but — being honest about what was and wasn't checked — NOT re-verified with another live purge (redundant/disruptive right after already doing one).
- **Dashboard: Health page no longer flickers to "(no containers found)" on a transient empty poll.** `HealthPage.apply_status()` previously trusted every poll result at face value — a `docker compose ps` invocation racing an in-flight Start/Restart for the same project (containers passing through `created`) could transiently come back empty, wiping a target's whole diagram group to the placeholder for a poll or two before self-correcting. Now debounced: an empty result for a target that had real containers a moment ago is treated as a blip for up to 2 consecutive polls (showing the last-known-good containers instead), and only redrawn as genuinely empty once the grace period elapses.
- **Dashboard: shared uptime timers for ids/siem on the Health page.** One live-ticking label per project (not per target — local and remote combined), computed from the earliest `CreatedAt` among that project's currently-running containers (parsed straight out of `docker compose ps`'s own JSON, no extra `docker` calls), ticking up every second between polls via a `QTimer` rather than only refreshing once per poll interval.
- **`[LOCATION / HEADER] route_decision=... threat_score=...` was mislabeled a nginx error on every single request.** `nginx.conf`'s location-level `header_filter_by_lua_block` logged this via `ngx.log(ngx.ERR, ...)`, unconditionally, on every request through `location /` -- including completely clean, successfully-routed ones (`route_decision='production' threat_score='0'`), which is not an error condition in any sense. The near-identical server-level `header_filter_by_lua_block` (`[HEADER_FILTER]`, same information, see §7's "two near-identical blocks" note) already correctly logs the same thing at `ngx.INFO` -- the two had just drifted to different severities, not a deliberate distinction. Downgraded both `ngx.ERR` calls in the location-level block to `ngx.INFO` to match. Root-cause fix, not a downstream workaround: nginx's own `[error]`/`[info]` severity tag is literally the word that made this trip the dashboard's error badge in the first place (its matcher just looks for the word "error" in the line, not a special-cased nginx severity check) -- fixing the severity at the source fixes both nginx's own log classification and the dashboard badge in one change. Verified live: the exact same request now logs `[info]`, and `ui.error_monitor.is_error_log_line()` no longer flags it. `nginx.conf` is bind-mounted live (confirmed via the Dockerfile's own comment -- an earlier session already fixed the build-vs-bind-mount gap this file used to have), so a plain container restart was enough; no image rebuild needed.
- **Dashboard: the Health page's error-badge log view no longer caps history at 300 lines.** Clicking a node's badge already filtered the tail down to error-containing lines client-side, but the underlying `docker compose logs` call still only fetched the last 300 lines of RAW (pre-filter) output -- for a chatty service where errors are a small fraction of total output, all of them could be pushed out of that window by unrelated noise, silently defeating the point of a view whose whole job is "show me every error." Now `--tail=all`; the plain (non-badge) log view is unchanged, still capped at 300 for readability.
- **Investigated (not a bug) — hours-long, steady stream of Suricata "High Rate HTTP Requests"/"Admin Panel Discovery"/"Honeypot Trigger - Rapid Vulnerability Scanning" alerts from `src_ip 172.21.0.10`.** Identified via `docker network inspect`: that address is `reverse_proxy` itself, on the `honeypot_network` side -- these are alerts on the reverse_proxy→backend hop of already-proxied traffic, not a separate attacker (matches an earlier session's own documented finding for exactly this address/mechanism; the "not attributed to IP reputation" log line is that existing fix correctly excluding this hop from `threat_ips`, not a symptom of anything broken). Confirmed via `fast.log`: 897 alerts on this internal hop vs. only 2 on the real client-facing one for the same 3 signatures -- consistent with a rate-based Suricata rule tripping on reverse_proxy's own aggregated traffic to each backend (an inherent property of proxying many clients through one upstream connection pattern, not a sign of an actual attack), most plausibly driven by the dashboard's own long-running embedded Exploits-page browser (same root cause identified for the SSL noise finding above). The `epoll_wait() failed (4: Interrupted system call)` lines in the same batch are standard, benign POSIX/nginx behavior (a signal interrupting a blocking syscall, retried immediately) at `[info]` severity. No code change made -- correctly-working, already-understood behavior, not a defect.
