...
**Why not `wp-cli`**: attempting to use it here (needed for scripted plugin install/activate) hit two unrelated bugs in immediate succession. First, `production-hardening.php`'s `rest_endpoints` filter assumed every entry under a REST route was a `methods`/`callback`/`permission_callback` array, but `WP_REST_Server::get_routes()` also stores a `'schema'` entry per route (a callable, not that shape) — `wp-cli` (via WooCommerce's own `WC_CLI_Runner::after_wp_load()` hook, which fires an internal REST-index warm-up on every single `wp` invocation once WooCommerce is active) is what actually exercises that path; real HTTP traffic never triggers it the same way. **Fixed**: the filter now skips non-array entries. Second, after that fix, a *different*, deeper crash surfaced inside WordPress core's own `WP_REST_Server::get_data_for_route()` (`call_user_func(): argument #1 must be a valid callback, array must have exactly two members`) — this one has nothing to do with the hardening mu-plugin (it's not in the stack trace) and looks like a genuine `WC_CLI_Runner`/WordPress-core/PHP 8.1 compatibility gap in this specific version combination. Not chased further since the actual goal (install and activate plugins) didn't need `wp-cli` at all — a ~40-line PHP script using `wp-load.php` + `ZipArchive` + `activate_plugin()` directly did the job and sidesteps `WC_CLI_Runner` entirely (it only hooks `wp-cli`'s own bootstrap action, never a plain PHP script). One thing that script run *did* catch: `get_plugins()`'s in-request object cache returned a stale (pre-extraction) snapshot for every plugin after the first one when download-and-activate ran as a single continuous script — fixed by calling `wp_cache_delete('plugins', 'plugins')` before each activation, or by splitting extraction and activation into separate process runs.
...
**Two Kibana dashboards built this session** (Saved Objects API, classic visualization type — 13 visualizations total), both live and querying real data:
...
**Still open**: these two cover the "IDS alerts" and "real-time threat overview" items from the originally-recommended list; "session analysis" and "attack patterns" (would need session-level aggregation — duration, request count per session, sophistication classification — none of which currently reaches Elasticsearch, only Redis, see §3.5) remain unbuilt — see §8.
...
### Still open
- ELK Dashboards (2 of 4 recommended dashboards built this session — IDS Alerts, Web Traffic & Threat Overview; session-analysis and attack-pattern dashboards still open, blocked on session-level data not currently reaching Elasticsearch — see §6)
