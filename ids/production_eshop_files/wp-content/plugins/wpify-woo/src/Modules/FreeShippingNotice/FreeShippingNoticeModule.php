...
class FreeShippingNoticeModule extends AbstractModule {
	...
	public function render_free_shipping_notice() {
		...
		if ( is_string( $filtered_html ) ) {
			...
			<div class="wpify-woo-free-shipping-notice__wrapper">
				<?php echo wp_kses_post( $filtered_html ); ?>
			</div>
			<?php
			return;
		}
		...
		<style type="text/css">
			...
			.progress-value {
				...
				border-radius: 100px;
				background: <?php echo esc_attr( $this->get_setting( 'progressbar_color' ) ?: '#fff' ); ?>;
				height: 5px;
				width: 0;
			}
			...
			@keyframes load {
				...
				100% {
					width: <?php echo esc_attr( $context['percentage_for_free_shipping'] ); ?>%;
				}
			}
		</style>
		<div class="wpify-woo-free-shipping-notice__wrapper">
			<div class="wpify-woo-free-shipping-notice" style="<?php echo esc_attr( $style ); ?>">
				<div>
					<?php if ( ! empty( $this->get_setting( 'show_icon' ) ) ) { ?>
						<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 122.88 61.03"
							...
							<path
								style="<?php echo esc_attr( $this->get_setting( 'icon_color' ) ? sprintf( 'fill: %s;', $this->get_setting( 'icon_color' ) ) : '' ); ?>"
				...
				<div>
					...
					<?php echo wp_kses_post( $context['free_shipping_message'] ); ?>
					<div class="progress">
	...
	public function get_free_shipping_amount() {
		...
		// phpcs:ignore WordPress.NamingConventions.PrefixAllGlobals.NonPrefixedHooknameFound -- Third-party WOOCS (Woo Currency Switcher) hook name.
