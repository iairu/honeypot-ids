...
class ActionScheduler_ActionFactory
{
     ...
     * @return int The ID of the stored action.
     */
    public function single_unique($hook, $args = array(), $when = null, $group = '', $unique = \true)
    {
     ...
     * @return int The ID of the stored action.
     */
    public function recurring_unique($hook, $args = array(), $first = null, $interval = null, $group = '', $unique = \true)
    {
        if (empty($interval)) {
     ...
     * @return int The ID of the stored action.
     **/
    public function cron_unique($hook, $args = array(), $base_timestamp = null, $schedule = null, $group = '', $unique = \true)
    {
        if (empty($schedule)) {
    ...
    {
        $defaults = array('type' => 'single', 'hook' => '', 'arguments' => array(), 'group' => '', 'unique' => \false, 'when' => \time(), 'pattern' => null, 'priority' => 10);
    ...
    protected function store_unique_action(\ActionScheduler_Action $action)
    {
        ...
        if (\method_exists($store, 'save_unique_action')) {
             ...
             */
            $existing_action_id = (int) $store->find_action($action->get_hook(), array('args' => $action->get_args(), 'status' => \ActionScheduler_Store::STATUS_PENDING, 'group' => $action->get_group()));
 ...
 * Class ActionScheduler_ActionFactory
 */
\class_alias('WpifyWooDeps\ActionScheduler_ActionFactory', 'ActionScheduler_ActionFactory', \false);
