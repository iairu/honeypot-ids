...
class BatchFetcher
{
    ...
    private function get_query_strategies($count)
    {
        ...
        $args = array('date' => $now, 'per_page' => $count, 'offset' => 0, 'orderby' => 'date', 'order' => 'ASC');
        $priorities = array(Store::STATUS_PENDING, Store::STATUS_FAILED, Store::STATUS_CANCELED, Store::STATUS_COMPLETE, Store::STATUS_RUNNING, '');
        foreach ($priorities as $status) {
            yield wp_parse_args(array('status' => $status, 'date_compare' => '<='), $args);
            yield wp_parse_args(array('status' => $status, 'date_compare' => '>='), $args);
        }
    }
}
