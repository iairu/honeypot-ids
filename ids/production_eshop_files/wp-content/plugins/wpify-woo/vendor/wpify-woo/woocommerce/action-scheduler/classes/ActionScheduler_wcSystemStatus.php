...
class ActionScheduler_wcSystemStatus
{
    ...
    protected function get_template($status_labels, $action_counts, $oldest_and_newest)
    {
        ...
        \esc_html_e('Action Scheduler', 'action-scheduler');
        echo \wc_help_tip(\esc_html__('This section shows details of Action Scheduler.', 'action-scheduler'));
        ?></h2></th>
				</tr>
				<tr>
					<td colspan="2" data-export-label="Version"><?php 
        \esc_html_e('Version:', 'action-scheduler');
        ?></td>
					<td colspan="3"><?php 
        echo \esc_html($as_version);
        ?></td>
				</tr>
				<tr>
					<td colspan="2" data-export-label="Data store"><?php 
        \esc_html_e('Data store:', 'action-scheduler');
        ?></td>
					<td colspan="3"><?php 
        echo \esc_html($as_datastore);
        ?></td>
				</tr>
				<tr>
					<td><strong><?php 
        \esc_html_e('Action Status', 'action-scheduler');
        ?></strong></td>
					<td class="help">&nbsp;</td>
					<td><strong><?php 
        \esc_html_e('Count', 'action-scheduler');
        ?></strong></td>
					<td><strong><?php 
        \esc_html_e('Oldest Scheduled Date', 'action-scheduler');
        ?></strong></td>
					<td><strong><?php 
        \esc_html_e('Newest Scheduled Date', 'action-scheduler');
        ?></strong></td>
				</tr>
			</thead>
			<tbody>
        ...
        foreach ($action_counts as $status => $count) {
            ...
            \printf('<tr><td>%1$s</td><td>&nbsp;</td><td>%2$s<span style="display: none;">, Oldest: %3$s, Newest: %4$s</span></td><td>%3$s</td><td>%4$s</td></tr>', \esc_html($status_labels[$status]), \esc_html(\number_format_i18n($count)), \esc_html($oldest_and_newest[$status]['oldest']), \esc_html($oldest_and_newest[$status]['newest']));
        }
        ?>
			</tbody>
		</table>
 ...
 * Class ActionScheduler_wcSystemStatus
 */
\class_alias('WpifyWooDeps\ActionScheduler_wcSystemStatus', 'ActionScheduler_wcSystemStatus', \false);
