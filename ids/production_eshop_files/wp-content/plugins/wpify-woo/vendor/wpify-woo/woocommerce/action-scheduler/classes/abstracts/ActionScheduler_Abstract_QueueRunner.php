...
abstract class ActionScheduler_Abstract_QueueRunner extends \ActionScheduler_Abstract_QueueRunner_Deprecated
{
    ...
    protected function schedule_next_instance(\ActionScheduler_Action $action, $action_id)
    {
        ...
        if (\ActionScheduler_Store::STATUS_FAILED === $this->store->get_status($action_id) && $this->recurring_action_is_consistently_failing($action, $action_id)) {
    ...
    private function recurring_action_is_consistently_failing(\ActionScheduler_Action $action, $action_id)
    {
        ...
        $query_args = array('hook' => $action->get_hook(), 'status' => \ActionScheduler_Store::STATUS_FAILED, 'date' => \date_create('now', \timezone_open('UTC'))->format('Y-m-d H:i:s'), 'date_compare' => '<', 'per_page' => 1, 'offset' => $consistent_failure_threshold - 1);
         ...
         */
        return (bool) \apply_filters('action_scheduler_recurring_action_is_consistently_failing', $first_action_id_with_the_same_hook === $first_failing_action_id, $action);
    }
    /**
    ...
    protected function get_execution_time()
    {
        ...
        if (\function_exists('getrusage') && \apply_filters('action_scheduler_use_cpu_execution_time', \defined('WpifyWooDeps\PANTHEON_ENVIRONMENT'))) {
 ...
 * Abstract class with common Queue Cleaner functionality.
 */
\class_alias('WpifyWooDeps\ActionScheduler_Abstract_QueueRunner', 'ActionScheduler_Abstract_QueueRunner', \false);
