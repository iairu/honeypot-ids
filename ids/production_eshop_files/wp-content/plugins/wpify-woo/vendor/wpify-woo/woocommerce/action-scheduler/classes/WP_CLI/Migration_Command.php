...
class Migration_Command extends WP_CLI_Command
{
    ...
    public function register()
    {
        ...
        WP_CLI::add_command('action-scheduler migrate', array($this, 'migrate'), array('shortdesc' => 'Migrates actions to the DB tables store', 'synopsis' => array(array('type' => 'assoc', 'name' => 'batch-size', 'optional' => \true, 'default' => 100, 'description' => 'The number of actions to process in each batch'), array('type' => 'assoc', 'name' => 'free-memory-on', 'optional' => \true, 'default' => 50, 'description' => 'The number of actions to process between freeing memory. 0 disables freeing memory'), array('type' => 'assoc', 'name' => 'pause', 'optional' => \true, 'default' => 0, 'description' => 'The number of seconds to pause when freeing memory'), array('type' => 'flag', 'name' => 'dry-run', 'optional' => \true, 'description' => 'Reports on the actions that would have been migrated, but does not change any data'))));
    }
    /**
    ...
    {
        $args = wp_parse_args($args, array('dry-run' => \false));
    ...
    private function init_logging()
    {
        ...
        add_action('action_scheduler/migration_batch_starting', function ($batch) {
            WP_CLI::debug('Beginning migration of batch: ' . print_r($batch, \true));
