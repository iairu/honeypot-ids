...
function as_enqueue_async_action($hook, $args = array(), $group = '', $unique = \false, $priority = 10)
{
    ...
    return \ActionScheduler::factory()->create(array('type' => 'async', 'hook' => $hook, 'arguments' => $args, 'group' => $group, 'unique' => $unique, 'priority' => $priority));
}
/**
...
function as_schedule_single_action($timestamp, $hook, $args = array(), $group = '', $unique = \false, $priority = 10)
{
    ...
    return \ActionScheduler::factory()->create(array('type' => 'single', 'hook' => $hook, 'arguments' => $args, 'when' => $timestamp, 'group' => $group, 'unique' => $unique, 'priority' => $priority));
}
/**
...
function as_schedule_recurring_action($timestamp, $interval_in_seconds, $hook, $args = array(), $group = '', $unique = \false, $priority = 10)
{
    ...
    return \ActionScheduler::factory()->create(array('type' => 'recurring', 'hook' => $hook, 'arguments' => $args, 'when' => $timestamp, 'pattern' => $interval_in_seconds, 'group' => $group, 'unique' => $unique, 'priority' => $priority));
}
/**
...
function as_schedule_cron_action($timestamp, $schedule, $hook, $args = array(), $group = '', $unique = \false, $priority = 10)
{
    ...
    return \ActionScheduler::factory()->create(array('type' => 'cron', 'hook' => $hook, 'arguments' => $args, 'when' => $timestamp, 'pattern' => $schedule, 'group' => $group, 'unique' => $unique, 'priority' => $priority));
}
/**
...
function as_unschedule_action($hook, $args = array(), $group = '')
{
    ...
    $params = array('hook' => $hook, 'status' => \ActionScheduler_Store::STATUS_PENDING, 'orderby' => 'date', 'order' => 'ASC', 'group' => $group);
    if (\is_array($args)) {
        $params['args'] = $args;
    }
    ...
    if ($action_id) {
        try {
            ...
            \ActionScheduler::logger()->log($action_id, \sprintf(
...
function as_has_scheduled_action($hook, $args = null, $group = '')
{
    ...
    $query_args = array('hook' => $hook, 'status' => array(\ActionScheduler_Store::STATUS_RUNNING, \ActionScheduler_Store::STATUS_PENDING), 'group' => $group, 'orderby' => 'none');
    if (null !== $args) {
...
function as_supports(string $feature): bool
{
    ...
    return \in_array($feature, $supported_features, \true);
}
