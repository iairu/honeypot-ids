...
class ActionScheduler_OptionLock extends \ActionScheduler_Lock
{
    ...
    public function set($lock_type)
    {
        ...
        if (empty($existing_lock_value)) {
            return (bool) $wpdb->insert($wpdb->options, array('option_name' => $lock_key, 'option_value' => $new_lock_value, 'autoload' => 'no'));
        }
        if ($this->get_expiration_from($existing_lock_value) >= \time()) {
            return \false;
        }
        // Otherwise, try to obtain the lock.
        return (bool) $wpdb->update($wpdb->options, array('option_value' => $new_lock_value), array('option_name' => $lock_key, 'option_value' => $existing_lock_value));
    }
    /**
    ...
    private function get_existing_lock($lock_type)
    {
        ...
        return (string) $wpdb->get_var($wpdb->prepare("SELECT option_value FROM {$wpdb->options} WHERE option_name = %s", $this->get_key($lock_type)));
    }
    /**
    ...
    {
        return \uniqid('', \true) . '|' . (\time() + $this->get_duration($lock_type));
    }
}
/**
 * Provide a way to set simple transient locks to block behaviour
 ...
 * @since 3.0.0
 */
\class_alias('WpifyWooDeps\ActionScheduler_OptionLock', 'ActionScheduler_OptionLock', \false);
