...
class ActionScheduler_DBStore extends \ActionScheduler_Store
{
    ...
    {
        return $this->save_action_to_db($action, $scheduled_date, \true);
    }
    /**
    ...
    private function build_where_clause_for_insert($data, $table_name, $unique)
    {
        ...
        $pending_statuses = array(\ActionScheduler_Store::STATUS_PENDING, \ActionScheduler_Store::STATUS_RUNNING);
    ...
    {
        $string_columns = array('hook', 'status', 'scheduled_date_gmt', 'scheduled_date_local', 'args', 'schedule', 'last_attempt_gmt', 'last_attempt_local', 'extended_args');
        return \in_array($column_name, $string_columns, \true) ? '%s' : '%d';
    }
    /**
    ...
    {
        if (!\in_array($select_or_count, array('select', 'count'), \true)) {
        ...
        if (!\is_null($query['args'])) {
            switch ($query['partial_args_matching']) {
                case 'json':
                    ...
                    $supported_types = array('integer' => '%d', 'boolean' => '%s', 'double' => '%f', 'string' => '%s');
                ...
                case 'like':
                    foreach ($query['args'] as $key => $value) {
                        ...
                        $json_partial = $wpdb->esc_like(\trim(\wp_json_encode(array($key => $value)), '{}'));
    ...
    public function cancel_action($action_id)
    {
         ...
         */
        global $wpdb;
        $updated = $wpdb->update($wpdb->actionscheduler_actions, array('status' => self::STATUS_CANCELED), array('action_id' => $action_id), array('%s'), array('%d'));
        if (\false === $updated) {
    ...
    protected function bulk_cancel_actions($query_args)
    {
        ...
        $action_ids = \true;
        $query_args = \wp_parse_args($query_args, array('per_page' => 1000, 'status' => self::STATUS_PENDING, 'orderby' => 'none'));
        while ($action_ids) {
    ...
    protected function claim_actions($claim_id, $limit, ?\DateTime $before_date = null, $hooks = array(), $group = '')
    {
        ...
        $where_params = array($date->format('Y-m-d H:i:s'), self::STATUS_PENDING);
        if (!empty($hooks)) {
        ...
        if (!empty($group)) {
            ...
            if (empty($group_ids)) {
                throw new \InvalidArgumentException(\sprintf(
                    ...
                    \_n('The group "%s" does not exist.', 'The groups "%s" do not exist.', \is_array($group) ? \count($group) : 1, 'action-scheduler'),
                    $group
                ));
            }
        ...
        $update_params = array($claim_id, $now->format('Y-m-d H:i:s'), \current_time('mysql'));
        $rows_affected = $wpdb->query($wpdb->prepare($update_sql, $update_params));
        if (\false === $rows_affected) {
            $error = empty($wpdb->last_error) ? \_x('unknown', 'database error', 'action-scheduler') : $wpdb->last_error;
            throw new \RuntimeException(\sprintf(
    ...
    private function db_supports_skip_locked()
    {
        ...
        if ($is_mariadb && '5.5.5' === $db_version && \PHP_VERSION_ID < 80016) {
            /*
        ...
        $is_supported = $is_mariadb && \version_compare($db_version, '10.6.0', '>=') || !$is_mariadb && \version_compare($db_version, '8.0.1', '>=');
        /**
    ...
    public function find_actions_by_claim_id($claim_id)
    {
        ...
        $sql = $wpdb->prepare("SELECT action_id, scheduled_date_gmt FROM {$wpdb->actionscheduler_actions} WHERE claim_id = %d ORDER BY priority ASC, attempts ASC, scheduled_date_gmt ASC, action_id ASC", $claim_id);
    ...
    public function unclaim_action($action_id)
    {
         ...
         */
        global $wpdb;
        $wpdb->update($wpdb->actionscheduler_actions, array('claim_id' => 0), array('action_id' => $action_id), array('%s'), array('%d'));
    }
    /**
    ...
    public function mark_failure($action_id)
    {
         ...
         */
        global $wpdb;
        $updated = $wpdb->update($wpdb->actionscheduler_actions, array('status' => self::STATUS_FAILED), array('action_id' => $action_id), array('%s'), array('%d'));
        if (empty($updated)) {
            /* translators: %s is the action ID */
    ...
    public function log_execution($action_id)
    {
        ...
        $sql = $wpdb->prepare($sql, self::STATUS_RUNNING, \current_time('mysql', \true), \current_time('mysql'), $action_id);
        // phpcs:ignore WordPress.DB.PreparedSQL.NotPrepared
        // phpcs:ignore WordPress.DB.PreparedSQL.NotPrepared
    ...
    public function mark_complete($action_id)
    {
         ...
         */
        global $wpdb;
        $updated = $wpdb->update($wpdb->actionscheduler_actions, array('status' => self::STATUS_COMPLETE, 'last_attempt_gmt' => \current_time('mysql', \true), 'last_attempt_local' => \current_time('mysql')), array('action_id' => $action_id), array('%s'), array('%d'));
        if (empty($updated)) {
            /* translators: %s is the action ID */
 ...
 * @since 3.0.0
 */
\class_alias('WpifyWooDeps\ActionScheduler_DBStore', 'ActionScheduler_DBStore', \false);
