...
abstract class ActionScheduler_Store extends \ActionScheduler_Store_Deprecated
{
    ...
    {
        $params = \wp_parse_args($params, array('args' => null, 'status' => self::STATUS_PENDING, 'group' => ''));
    ...
    {
        $extra_actions = array();
        $pastdue_action_counts = (int) $this->query_actions(array('status' => self::STATUS_PENDING, 'date' => \as_get_datetime_object()), 'count');
    ...
    {
        if (\in_array($comparison_operator, array('!=', '>', '>=', '<', '<=', '='), \true)) {
    ...
    public function cancel_actions_by_hook($hook)
    {
        ...
        while (!empty($action_ids)) {
            $action_ids = $this->query_actions(array('hook' => $hook, 'status' => self::STATUS_PENDING, 'per_page' => 1000, 'orderby' => 'none'));
    ...
    public function cancel_actions_by_group($group)
    {
        $action_ids = \true;
        while (!empty($action_ids)) {
            $action_ids = $this->query_actions(array('group' => $group, 'status' => self::STATUS_PENDING, 'per_page' => 1000, 'orderby' => 'none'));
            $this->bulk_cancel_actions($action_ids);
        }
    }
    /**
    ...
    {
        return array(self::STATUS_COMPLETE => \__('Complete', 'action-scheduler'), self::STATUS_PENDING => \__('Pending', 'action-scheduler'), self::STATUS_RUNNING => \__('In-progress', 'action-scheduler'), self::STATUS_FAILED => \__('Failed', 'action-scheduler'), self::STATUS_CANCELED => \__('Canceled', 'action-scheduler'));
    }
    /**
    ...
    {
        $pending_actions = $this->query_actions(array('per_page' => 1, 'date' => \as_get_datetime_object(), 'status' => self::STATUS_PENDING, 'orderby' => 'none'), 'count');
 ...
 * @codeCoverageIgnore
 */
\class_alias('WpifyWooDeps\ActionScheduler_Store', 'ActionScheduler_Store', \false);
