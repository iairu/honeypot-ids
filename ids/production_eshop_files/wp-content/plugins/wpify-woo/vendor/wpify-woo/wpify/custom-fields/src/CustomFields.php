...
use WpifyWooDeps\Wpify\CustomFields\Integrations\Options;
use WpifyWooDeps\Wpify\CustomFields\Integrations\OrderMetabox;
use WpifyWooDeps\Wpify\CustomFields\Integrations\ProductOptions;
use WpifyWooDeps\Wpify\CustomFields\Integrations\ProductVariationOptions;
use WpifyWooDeps\Wpify\CustomFields\Integrations\SiteOptions;
use WpifyWooDeps\Wpify\CustomFields\Integrations\SubscriptionMetabox;
use WpifyWooDeps\Wpify\CustomFields\Integrations\Taxonomy;
use WpifyWooDeps\Wpify\CustomFields\Integrations\User;
use WpifyWooDeps\Wpify\CustomFields\Integrations\WcMembershipPlanOptions;
use WpifyWooDeps\Wpify\CustomFields\Integrations\WooCommerceSettings;
/**
 * Class CustomFields
 *
...
class CustomFields
{
     ...
     */
    public const FIELD_CAPABILITIES = array('direct_file' => 'upload_files', 'multi_direct_file' => 'upload_files', 'cloudflare' => 'manage_options');
    /**
     * Helpers class.
     *
     * @var Helpers
    ...
    public function sanitize_item_value(array $item): Closure
    {
        ...
        return function (mixed $value, mixed $previous_value = null) use ($item): mixed {
            ...
            } elseif (in_array($item['type'], array('wysiwyg', 'richtext'), \true)) {
                $sanitized_value = wp_kses_post($value);
            } elseif (in_array($item['type'], array('multi_checkbox', 'multi_toggle'), \true)) {
                $value = is_string($value) ? json_decode($value, \true) : (array) $value;
                $sanitized_value = array();
                if (is_array($value)) {
    ...
    {
        if (in_array($item['type'], array('attachment', 'post', 'term'), \true)) {
            $type = 'integer';
        } elseif (in_array($item['type'], array('number', 'range'), \true)) {
            $type = 'number';
        } elseif (in_array($item['type'], array('checkbox', 'toggle'), \true)) {
