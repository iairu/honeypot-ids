...
class WooCommerceSettings extends OptionsIntegration
{
    ...
    public function __construct(array $args, CustomFields $custom_fields)
    {
        ...
        $this->id = $args['id'] ?? join('_', array('wc_settings_', $this->tabs['id'] ?? '', $this->section['id'] ?? '', wp_generate_uuid4()));
        ...
        if ($this->is_new_tab) {
            ...
            if (is_array($sections) && count($sections) > 1) {
				...
				<ul class="subsubsub">
					<?php 
                foreach ($sections as $id => $label) {
                    $url = add_query_arg(array('page' => 'wc-settings', 'tab' => rawurlencode($this->tab['id']), 'section' => sanitize_title($id)), admin_url('admin.php'));
                    ?>
						<li>
							<a href="<?php 
                    echo esc_url($url);
    ...
    public function save(): void
    {
        ...
        if ($tab === $this->tab['id'] && $section === $this->section['id']) {
            ...
            wp_safe_redirect(add_query_arg(array('page' => 'wc-settings', 'tab' => $this->tab['id'], 'section' => $this->section['id'], 'settings-updated' => \true), admin_url('admin.php')));
            exit;
        }
    }
    /**
