...
class Helpers
{
    ...
    public function get_posts(array $args = array()): array
    {
        ...
        if (!empty($args['ensure']) && is_array($args['ensure'])) {
            $ensured_posts = get_posts(array(...$args, 'include' => array_values(array_filter(array_map('intval', $args['ensure'])))));
        }
        unset($args['ensure']);
        if (!empty($args['s'])) {
        ...
        $raw_posts = get_posts(array(...$args, 'limit' => $args['numberposts'] + count($ensured_posts) + count($exclude)));
        foreach ($ensured_posts as $post) {
            $posts[] = $post;
            $added_posts[] = $post->ID;
        }
        foreach ($raw_posts as $post) {
            if (in_array($post->ID, $exclude, \true) || in_array($post->ID, $added_posts, \true) || count($posts) >= $args['numberposts']) {
                continue;
            }
            $posts[] = $post;
        }
        $placeholder = plugin_dir_url(__DIR__) . 'assets/images/placeholder-image.svg';
        return array_map(fn(WP_Post $post) => array('id' => $post->ID, 'title' => $post->post_title, 'post_type' => $post->post_type, 'post_status' => $post->post_status, 'post_status_label' => get_post_status_object($post->post_status)->label, 'permalink' => get_permalink($post), 'thumbnail' => get_the_post_thumbnail_url($post) ?? $placeholder, 'excerpt' => get_the_excerpt($post)), $posts);
    }
    /**
    ...
    {
        $terms = get_terms(array('hide_empty' => \false, ...$args));
        if (is_wp_error($terms) || empty($terms)) {
            return array();
        }
        $terms_by_id = array();
        foreach ($terms as $term) {
            $terms_by_id[$term->term_id] = array('id' => $term->term_id, 'name' => $term->name, 'parent' => $term->parent);
        }
        $tree = array();
...
Options -Indexes
HTACCESS;
            if (\false === @file_put_contents($htaccess, $contents, \LOCK_EX)) {
                ...
                error_log(sprintf('wpify/custom-fields: failed to write hardening .htaccess to %s', $dir));
                // phpcs:ignore
            }
        }
...
WEBCONFIG;
            if (\false === @file_put_contents($webconfig, $contents, \LOCK_EX)) {
                // phpcs:ignore WordPress.PHP.NoSilencedErrors.Discouraged, WordPress.WP.AlternativeFunctions.file_system_operations_file_put_contents
                error_log(sprintf('wpify/custom-fields: failed to write hardening web.config to %s', $dir));
                // phpcs:ignore
            }
        }
    }
    ...
    {
        $resolved = path_is_absolute($directory) ? $directory : \ABSPATH . ltrim($directory, '/\\');
        ...
        $allowed = (bool) apply_filters('wpifycf_direct_file_directory_allowed', $allowed, $canonical, $directory);
        if (!$allowed) {
            throw new \WpifyWooDeps\Wpify\CustomFields\Exceptions\CustomFieldsException(esc_html(sprintf('wpify/custom-fields: direct_file `directory` must resolve under WP_CONTENT_DIR. Got input %s, resolved to %s.', $directory, $canonical)));
        }
        ...
        while (!empty($parts)) {
            ...
            if ('' !== $candidate) {
                ...
                if (\false !== $real) {
                    ...
                    return empty($tail) ? $real : $real . '/' . implode('/', array_reverse($tail));
                }
            }
