...
class SubscriptionMetabox extends ItemsIntegration
{
    ...
    public function __construct(array $args, CustomFields $custom_fields)
    {
        ...
        $this->id = sanitize_title(join('-', array('order-meta', $this->hook_priority, $this->title)));
    ...
    public function add_meta_box(string $post_type): void
    {
        ...
        $screen = wc_get_container()->get(CustomOrdersTableController::class)->custom_orders_table_usage_is_enabled() ? wc_get_page_screen_id('shop-subscription') : 'shop_subscription';
        if ($screen !== $post_type) {
            return;
        }
    ...
    public function render(WC_Order $order): void
    {
        ...
        $this->print_app('subscription-meta', $this->tabs, array(), $prepared);
    }
    /**
