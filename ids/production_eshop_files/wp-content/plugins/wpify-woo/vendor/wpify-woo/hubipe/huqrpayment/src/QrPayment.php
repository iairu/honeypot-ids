...
namespace WpifyWooDeps\hubipe\HuQrPayment;

use WpifyWooDeps\hubipe\HuQrPayment\Config\Configuration;
use WpifyWooDeps\hubipe\HuQrPayment\Config\ConfigurationInterface;
use WpifyWooDeps\hubipe\HuQrPayment\Enums\CharacterSet;
use WpifyWooDeps\hubipe\HuQrPayment\Enums\IdCode;
use WpifyWooDeps\hubipe\HuQrPayment\Enums\Purpose;
use WpifyWooDeps\hubipe\HuQrPayment\Exceptions\InvalidIbanException;
use WpifyWooDeps\hubipe\HuQrPayment\Exceptions\InvalidOptionException;
use WpifyWooDeps\hubipe\HuQrPayment\Helper\Utils;
...
class QrPayment implements QrPaymentInterface
{
    ...
    public function getQrString(): string
    {
        ...
        $dueDate = sprintf('%s+%d', $this->dueDate->setTimezone($localTimeZone)->format('YmdHis'), round($localTimeZone->getOffset($this->dueDate) / 3600));
        $result = [];
        ...
        if ($byteLength > 345) {
            throw new LogicException(sprintf('The resulting QR string is limited to 345 bytes, yours has %d bytes', $byteLength));
        }
        return $result;
    }
    ...
    private function checkLength(string $string, int $max, int $min = 0): void
    {
        ...
        if ($length > $max || $length < $min) {
            throw new InvalidArgumentException(sprintf('The string should be between %d and %d characters long, your string contains %d characters.', $min, $max, $length));
        }
    }
    private function checkCharacters(string $string): void
    {
        if (strpos("\n", $string) !== \FALSE) {
            throw new InvalidArgumentException(sprintf('The string "%s" contains forbidden new line character.', $string));
        }
    }
    /**
